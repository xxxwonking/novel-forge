...
当前已接入作品列表、资料方案、试写、结果页、后台任务、手动编辑、结构纠错、自然语言局部改写、保留片段继续完成、至多一次自动修订、C4 建议随稿采用、故事进度和规划伏笔生命周期。固定版本的基础导出继续开发。并发协调限定在同一服务进程的项目会话内。L2 目前每次根据正式事件重建；生产环境的增量快照与真实缓存收益仍需后续验证。
