...
人物、情节线和地点引用必须使用已确认 ID；姓名冒充 ID、未知引用或未支持的状态字段会使 C5 失败，不能靠过滤记录获得采用资格。`characterStates.field` 只支持 `condition`、`location`、`vital`：持有物变化写入完整的 `condition`，`location` 使用地点 ID，`vital` 使用 `alive/dead/missing/unknown`；没有原值时 `from` 为 null。生成与纠错共用这些校验；合法非数字地点编号同样进入人物所在地。恢复失败的 C5 保留已完成正文，只重新执行必要的结构核对。
...
当前已接入作品列表、资料方案、试写、结果页、后台任务、手动编辑、结构纠错、自然语言局部改写、保留片段继续完成、至多一次自动修订、C4 建议随稿采用、故事进度、规划伏笔生命周期和固定正式版本 TXT 导出。并发协调限定在同一服务进程的项目会话内。L2 目前每次根据正式事件重建；生产环境的增量快照与真实缓存收益仍需后续验证。
...
`GET /api/issues` 返回进度数组，`GET /api/alerts` 的 `progress` 返回相同内容；主 Agent 工具 `get_story_progress` 读取同一业务入口。每项包含 `id/kind/title/state/detail`、可选期限 `expectedBy`、后续 `arrangements`、正式原文 `evidence`、`history` 及当前可用的 `actions`。查询不写入作品，不调用模型。

状态区分 `planned`（尚未埋设的规划）、`pending`、`scheduled`、`rescheduled`、`partial`、`resolved`、`abandoned`、`exited`。部分兑现继续跟踪，只有可定位的正式原文支持完全兑现时才显示已兑现；情节/人物的完成只表示本次安排满足，不代表故事线结束。作者确认退场单独展示，若仍有未来出场计划会明确指出需核对；后来采用的新出场记录按正文更新进度。
...
C5 的 `foreshadow_planted` 条目支持 `planned_foreshadow_id`；也可通过唯一且完全相同的规划标签关联。候选不改变规划状态，采用后沿用原编号与真实原文锚点；非法、歧义或重复关联及重复兑现使 C5 明确失败，保留正文。结构纠错除单项检查外，还会重新验证整份声明的交叉约束。

某项未来埋设/回收已由之前采用的正文完成时，写章按剩余计划计算预算，并在上下文说明已完成项，避免重复埋设或兑现。作者原计划不被删除；最新章修订撤销该事实后，原待办自动恢复。缺少有效原文的完成记录不会静默清除计划。P4 规划不进入正式人物状态、出场、情节推进或关系；关系再次变化后，视图原文入口指向最新变化。

## 固定正式版本 TXT 导出

`POST /api/export/preview` 接受 `{ "scope": "all" }` 或 `{ "scope": "range", "from": 1, "to": 3 }`。章号须为有序的正安全整数；全部模式不能夹带起止章号。返回固定的 `id/title/filename/selection/chapters/omitted/pendingDrafts/totalWords/createdAt/sha256/message`。

`chapters` 按章号排序，每项包含 `chapter/draftId/version/words/sha256`。有采用映射的正文必须与记录相符；历史正文无版本映射时，`draftId` 为 null，`version` 为 `legacy:<正文 SHA256>`。`omitted` 用 `{ from, to }` 区间表示所选范围缺失的正文；`pendingDrafts` 列出未采用稿所在章节与数量，候选内容不进入 TXT。没有可导出正文时返回 HTTP 200 和 `id: null/filename: null/chapters: []`，提供调整说明，不创建空文件。

`GET /api/export?id=<id>` 重读固定预览。`GET /api/export/file?id=<id>` 返回 JSON `{ filename, text, sha256 }`，Web 用 UTF-8 Blob 下载 TXT，文件含 BOM、书名和章节分隔；文件名去除 Windows 非法字符。预览、读取与下载均不需要模型配置。

清单和 TXT 通过文件事务存到当前作品的 `exports/export-<64位指纹>.json/.txt`，正文不写进清单。相同内容与选择重试复用原快照；预览后采用修订、增加章节或重开作品，都不会改变原文件。读取校验清单指纹与文本哈希；非法输入返回 400，当前作品没有该导出返回 404，版本不一致或文件损坏/缺失返回 409。损坏快照的同范围重试也不会用当前正文静默替换原文件。

Agent 工具 `prepare_text_export` 使用同一输入和服务，成功产生 `{ kind: "export_prepared", exportId, chapters }`，对话中的链接打开 `#/export?id=...`。空范围返回带说明的 `action_failed`，不会追加写章或采用。Web 修改表单后仍下载已展示的预览，重新预览后才切换版本；切换页面会使旧请求的响应失效。
