...
可以用 `npm run serve -- data/my-library 5174` 指定工作区和端口；传入已有作品目录时直接打开该作品。作品数据保存在本地目录，不纳入 Git，请自行备份。
...
本地服务当前仅监听 `127.0.0.1`，尚未提供用户登录。访问 Host 须为回环地址；携带 Origin 的请求须与 Host 同源，API POST 须使用 `application/json`，请求体上限 4 MiB。`npm run acceptance:m1` 是单独的真实模型验收流程，会调用模型；它不属于上面的演示与常规检查。
