...
export class ChapterTaskService {
  ...
  private draftFromState(identity: DraftIdentity, state: ChapterGraphState): ChapterDraft {
    return {
      ...
      ...(state.voiceForBody === null ? {} : { voiceCheck: { forBody: state.voiceForBody, findings: state.voiceFindings } }),
      ...(state.semanticsForBody === null ? {} : { semanticCheck: { forBody: state.semanticsForBody, findings: state.semanticsFindings } }),
...
function stateFromDraft(draft: ChapterDraft, runInput: ChapterRunInput): ChapterGraphState {
  ...
  return {
    ...
    voiceForBody: draft.voiceCheck?.forBody ?? null,
    voiceFindings: draft.voiceCheck?.findings ?? [],
    semanticsForBody: draft.semanticCheck?.forBody ?? null,
    semanticsFindings: draft.semanticCheck?.findings ?? [],
