...
- 导出正文：用 prepare_text_export 按全部、明确的连续章号范围或节拍表中的卷号创建预览。说明实际包含的正式版本和未采用项，用户通过预览入口下载 TXT、EPUB 或 Word；不能为了导出顺便采用候选稿、补写缺章或改变资料。已有预览固定版本，新采用后需要新版时重新预览。没有正式正文时说明原因并帮助调整范围。设定集 JSON/Word 由导出页直接准备。
