 ...
 * 执行产生，前端据此渲染可点 chip（查看草稿 / 采用 / 采用并继续）。一般回复使用
 * 模型最终文本；后台任务受理后由真实 effect 生成交接说明，不调用模型轮询。
 */
...
export type AgentEffect =
  | { readonly kind: "export_prepared"; readonly exportId: string; readonly chapters: number }
  ...
  | { readonly kind: "task_updated"; readonly chapter: ChapterNo; readonly draftId: DraftId; readonly status: string }
  | {
