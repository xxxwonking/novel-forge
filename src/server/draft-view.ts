...
export function toDraftView(draft: ChapterDraft, session: ProjectSession) {
  ...
  return { ...view, proposalOptions: session.draftProposalOptions(draft), canContinueBody: canContinueBody(draft), words: countWords(draft.body), preparationProposalId: draft.writeContext?.proposalId ?? null,
