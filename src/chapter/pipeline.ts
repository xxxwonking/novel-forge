...
/**
 * C7 的修订指令。作为同会话后续一轮追加，接在 C4 之后（不带 C5）——
 * 修订完正文变了，结构声明必然要重做，所以没必要把旧声明带进上下文误导。
 *
 * 「只改被指出的地方」是这段的核心：不约束的话模型会顺手重写全章，
 * 作者上一版满意的段落被悄悄换掉，新旧对比也就失去意义。
 */
export const C7_TASK = [
  "这一章没有通过检查。按下面的问题清单修订后，重新输出完整正文。",
  "1. 只改问题指向的地方，没被指出的段落**一字不动**。不要借机重写。",
  "2. 问题里给了补写/删减优先级的，严格按那个顺序做；明确禁止的东西一个字都不要加。",
  "3. 改完仍要落在字数预算内，且不能破坏已埋或已收的伏笔。",
  "4. 只输出修订后的完整正文，不要说明、不要对比、不要标注改了哪里。",
].join("\n");
