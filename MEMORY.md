...
| 7 | 删除与恢复入口、备份/迁移 | ⬜ **队列头** ⚠ **含四件事，动手前先跟用户切范围**（作品删除／草稿删除／资料条目删除（§21 末就欠着）／备份迁移） |
| 8 | 导出格式（现只有固定版本 TXT） | ⬜ |
| 9 | 全文检索 | ⬜ |
...
1. **差距第 7 项「删除与恢复入口、备份/迁移」= 队列头。** ⚠ 这一项含四件事（作品删除 / 草稿删除 / **资料条目删除**（§21 末就欠着）/ 备份迁移），按本节「范围超出一批就先问」的规矩，**动手前要先跟用户切范围**。
