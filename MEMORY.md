...
### 第 2 项「导入旧作」的起手点（还没动手，仅为省下重新摸索的时间）

先核对这几处能不能复用，再决定方案：

- **抽取**：`src/chapter/c5-schema.ts` 的 C5 声明机制本来就是「从正文得出结构」，
  导入要的正是同一件事，只是方向从"写完后声明"变成"读旧稿反推"。
- **落点**：第 22 节的「AI 起草资料、作者只审」已经把"AI 产出 → 候选方案 → 作者确认"
  这条路打通了（`PreparationService` + `/api/preparation/draft`）。导入很可能是它的
  一个新入口，而不是另起一套。
- **难点**：长篇要分批（上下文装不下整本书），且章与章之间的结构要能累积；
  伏笔的"埋设→兑现"跨章关系尤其如此。这是本项真正的设计问题。
- **用户原话**（第 22 节）：「作者可能没看过这本小说，他也不清楚具体人物性格」——
  导入场景下这句最尖锐，AI 必须读正文反推，作者只确认。
