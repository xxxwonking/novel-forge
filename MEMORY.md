...
| 5 | 批量/连续创作 与 跨多章返修 | 🔵 **连续创作已完成（第 33 节，PR #10）；跨多章返修仍未做 —— 队列头** |
| 6 | 卷/部结构（`Beat.volume` 已有字段，无入口） | ⬜ |
...
**分支与 PR**
- `master` = `a3b193c`（2026-09-18 合完三条：逐章反推结构 PR #6、首页新建改版 PR #9、连续创作 PR #10）。远端只剩 `master` 与四条历史分支（stage1/stage2 那批，留着不删）。
...
- 工程状态：66 文件 / 1096 测试过；typecheck（根+web）、web:build、`git diff --check` 干净。
...
**待办（按序）**
1. **差距第 5 项下半场「跨多章返修」** —— 队列头。改早章后要定位受影响段落并给修订建议，现在只把后续稿标过期。
2. **流式响应的空闲超时**（第 33 节末的发现）：真机一次调用挂了 940 秒。连写让这个问题更痛，建议排在返修前后都行。
3. **连写的真机 live 未跑**（§33）：排章已在 gemini-3-flash 上验证过，连写本身只在脚本化替身上跑通。**Mac 有 `.env.local`**，别再写成只有 Windows 有。
4. 差距第 6–10 项按队列顺序（卷结构 → 删除备份 → 导出格式 → 全文检索 → 成本账）。
