...
| 7 | 删除与恢复入口、备份/迁移 | 🟡 **作品删除 + 草稿恢复已完成**（第 38 节，2026-09-19，范围由用户当场切定）；**仍欠资料条目删除（§21 末）与备份/迁移** |
| 8 | 导出格式（现只有固定版本 TXT） | ⬜ |
| 9 | 全文检索 | ⬜ |
...
**分支与 PR**
- `master` = `748877e`（merge PR #12，调用时长记两个钟 §37 —— 不改超时，只是定位手段）。其前是 `1657c8c`（PR #11 一条带走跨多章返修 §34、整体排查三修 §35、卷/部结构 §36）。
- **当前分支 `work-deletion-restore`**（base master，不栈）：删除与恢复入口（§38）。
- **API 链路坏时合 PR 的备用路已再次验证**：`gh` 走的是同一条坏链路无法绕过，本地 `git merge --no-ff` 后用 SSH 直连 IP 推 master，GitHub 会自动把 PR 标成 Merged（PR #12 就是这么合的）。
- 远端已 prune 掉合完的分支；留着的 `stage1-*` / `stage2-*` 是历史线，不删。
- 工程状态：**1174 测试过**（1 skipped），typecheck（根+web）、web:build、`git diff --check` 干净。
...
1. **差距第 7 项的后两件事 = 队列头**：**资料条目删除**（§21 末就欠着：要扩 `PREPARATION_CHANGES_SCHEMA` 与 `build()`，难点全在引用完整性 —— 被节拍表 `characters`/`locations`/`plotLine` 引用、被他人 `speech.addressForms[].target` 引用、被已采用正文引用的都不能删）与**备份/迁移**（§38 的归档目录已经是「一本完整的作品」，打包/还原可以直接架在它上面）。前者独立一批，后者独立一批。
2. 其后：第 8 项导出格式 → 第 9 项全文检索 → 第 10 项成本账（受凭证限制）。
...
| 闸门 | 不设会怎样 |
| --- | --- |
| 有章节任务在跑 / 正在连写 → 409 | 任务继续写向一个已经改名的目录 |
