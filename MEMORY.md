...
| 7 | 删除与恢复入口、备份/迁移 | 🟡 四件事已完成三件：作品删除 + 草稿恢复（§38，PR #13）、资料条目删除（§39，本分支）。**只剩备份/迁移** |
| 8 | 导出格式（现只有固定版本 TXT） | ⬜ |
| 9 | 全文检索 | ⬜ |
...
**分支与 PR**
- `master` = `1657c8c`（merge PR #11）。**PR #11 一条带走三批**：跨多章返修（§34）、整体排查后的三项修复（§35）、卷/部结构（§36）。合并时把标题与正文改成了名实相符的 —— 原标题只写「跨多章返修」，照原样合下去 PR 历史会永久低报这条分支的内容。
- **PR #12 open**：分支 `call-timing`，调用时长记两个钟（§37）。**不改超时，只是定位手段。**
- 远端已 prune 掉三条合完的分支；留着的 `stage1-*` / `stage2-*` 是历史线，不删。
- 工程状态：**1160 测试过**（1 skipped），typecheck（根+web）、web:build、`npm audit --omit=dev` 0 漏洞。
...
1. **差距第 7 项只剩「备份/迁移」= 队列头。** 前三件已完成：作品删除 + 草稿恢复（§38，PR #13）、资料条目删除（§39）。备份/迁移可以直接架在 §38 的归档目录上 —— 那个目录本来就是「一本完整的作品」，打包与还原是同一套改名/复制动作。
...
7. **改判定逻辑时做变异测试**：把刚加的规则注掉，确认对应测试确实变红。这几批
   抓住过「兑现优先于埋设」「卷纲顶替」「跨区不顶替」三条（§35、§36）。
...
> 编号说明：§38 是同期另一条分支（PR #13，删除与恢复入口）的记录。两条分支各自从 master 起，互不依赖，合并时这一节可能要跟着顺延。
