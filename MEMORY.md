...
| 2 | 导入旧作 | ✅ 正文入库第 29 节；逐章反推结构第 31 节（Mac 会话，2026-09-17，PR #6） |
...
| 7 | 删除与恢复入口、备份/迁移 | ⬜ **队列头**（先做流式超时的定位也行，见第 35 节末）（注：资料条目的**删除**也还没做，见第 21 节末） |
| 8 | 导出格式（现只有固定版本 TXT） | ⬜ |
| 9 | 全文检索 | ⬜ |
| 10 | 成本账（官方 key / DeepSeek 凭证缺失） | ⬜ 受凭证限制 |
...
### 当前进展快照（2026-09-18，三条 PR 合入 master 之后）

**分支与 PR**
- `master` = `7df8960`（2026-09-18 合完三条：逐章反推结构 PR #6、首页新建改版 PR #9、连续创作 PR #10）。其上开了 `cross-chapter-revision` → **PR #11 open**（跨多章返修，第 34 节，base master、不栈）。远端另有四条历史分支（stage1/stage2 那批，留着不删）。
- **栈式 PR 的教训**：base 指向另一条特性分支的 PR，在基分支随上一条合入被删时会被 GitHub **直接关闭**（不是自动改基），且关闭后无法重开、无法改 base，只能用同一分支新开一条。所以 PR #7 → #9、PR #8 → #10。**下次要么不栈，要么合之前先手动把下一条的 base 改到 master。**
- 工程状态：71 文件 / 1113 测试过（PR #11 之上）；typecheck（根+web）、web:build、`git diff --check` 干净。
...
- **连写里的自动采用是「逐章采用」的唯一例外**（§33）：只在作者显式授权一个章号范围、且该章检查通过并**无关键变化**时发生。关键变化 = 人物生死 / 关系变化 / 主线伏笔埋设 / 写作中提出的人物设定建议，命中就停在那一章交给作者。连写串行不并行；停下只在章与章之间生效。
- **旧稿反推结构顺序进行、产物只进 proposed**（§31）：后章要兑现的伏笔编号由前章分配，服务端拒绝跳章反推与先确认后章；引文必须逐字在正文里（唯一硬闸门），一章一视角这类写作纪律对旧稿只报 warn。
- **想法可以留空**（§32）：空想法建空白作品并直接进谋篇模式，`{}`／只有书名／想法留空都是合法输入。
- **跨章返修：代码判硬矛盾、模型找段落，不靠情节线扩散**（§34）。未处理的硬矛盾只挡连写不挡单章手写；产物是建议不是改稿；一个受影响章一条记录，合并态指纹一变「已处理」自动失效。早章返修由此解锁。

**待办（按序）**
1. **流式响应的空闲超时**（第 33 节末的发现）—— 队列头。真机一次调用挂了 940 秒，连写与逐章定位都让它更痛。修法：给流式读取加空闲超时，超时按 incomplete 处理。
2. 差距第 6–10 项按队列顺序（卷结构 → 删除备份 → 导出格式 → 全文检索 → 成本账）。
3. **两处真机 live 未跑**：连写本身（§33，排章已在 gemini-3-flash 上验证过）与跨章返修的定位通道（§34，只在脚本化替身上跑通）。**Mac 有 `.env.local`**，别再写成只有 Windows 有。
4. 第 19 节三项验收仍未做：作者要求传递修复后的真实 C4 复测、三章正文人工核对、用户流程 §10 的 16 项审计。
5. 对外暴露前加登录（服务端无鉴权，只绑 127.0.0.1）。
6. M1 缓存命中率实测仍卡官方直连 key。
...
`~/.claude/jobs/nf-clock/probe.mts` 在后台跑着（`drift.log`），只在两个钟这一拍对不上超过 2 秒时记一行。等这台机器自然睡一次（按 pmset 记录，闲置约 15 分钟就会睡），就能知道 **Darwin 上 `AbortSignal.timeout` 计的那个钟在系统睡眠期间是否继续走**。下次接手先看那个日志。

不能用 SIGSTOP 代替：进程暂停时 Darwin 的 `CLOCK_MONOTONIC` 照常走，系统睡眠时才冻结，两者测的不是一回事。

### 下一步

拿到钟差结论再定改什么。若确认单调钟在睡眠期间冻结，则超时行为本身是对的，要补的是**墙钟侧的兜底**（作者等了很久却看不到任何交代）；若两个钟在睡眠期间都走，那 940 秒另有原因，得靠新落的 `timing` 等它复现。
