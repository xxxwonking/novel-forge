...
**分支与 PR**
- `master` = `93d4a83`（PR #5 谋篇模式已合入，远端分支已删）。
- `import-structure-inference`（逐章反推结构，**PR #6 open**，base master，待 review 合入）。
- `works-first-screen`（首页新建作品改版，**PR #7 open**，base `import-structure-inference`，栈在 PR #6 之上；PR #6 合入后 GitHub 会自动改基到 master）。
- PR #3 / #4（Mac 旧线：多作品工作区、自动修订、三栏对话、稿本仪器样式、旧版谋篇）**已关闭作废**，其功能已由 master 等价实现覆盖或被 antd 界面取代；分支留在远端不删。
- 工程状态：63 文件 / 1086 测试过；typecheck（根+web）、web:build、`git diff --check` 干净。
...
**待办（按序）**
1. **review 并合入 PR #6、PR #7**（按栈顺序）；合后删两条远端分支。
2. **差距第 5 项「批量/连续创作 与 跨多章返修」** —— 队列头。动手前按第 28 节的两种例外先问范围：两个子项明显超出一批。
3. 谋篇模式与逐章反推的真机 live（Gemini 配置在 Windows 机；Mac 无 `.env.local`）。反推的提示词只在脚本化替身上验证过，真模型是否会复述而非逐字引用要看真机。
...
- [ ] 反推的**真机 live**：脚本化替身证明的是链路，真模型会不会复述而非逐字引用、伏笔 intent 写得够不够具体，要看真机。
...
### 未做
- [ ] 真机 live 看谋篇模式在**完全空白作品**上的第一轮回复是否真的按「一句话冲动 → 主角 → 冲突」带人（提示词是 §30 写的，当时验证用的作品有一句想法）。
