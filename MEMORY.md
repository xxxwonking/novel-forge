...
更新时间：2026-09-21（Asia/Shanghai；**差距清单十项全部完成，导入旧稿整条线做通**；`master` = `4c30080`，PR #23 待合；队列头 = 让弧线/伏笔/情节线三张视图在导入后也有内容；**接手先读第 28 节的「当前进展快照」**；“继续完成项目”总目标保持暂停）

记录范围：本文件汇总项目调研、产品决策与开发进展。**接手先读第 28 节**（当前分支、队列、持续授权与工作节奏），再按队列项读对应批次记录；**本轮（2026-09-21）的概览见第 49 节**，删除与恢复见第 38 节，资料条目删除见第 39 节。Gemini 真实验收《雾港封签》三章（ch1d3 / ch2d4 / ch3d4，共 9745 字）已逐章采用并完成 1–3 章固定版本 TXT 导出，累计 62 次真实调用。尚未完成：修复后的真实 C4 复测、三章正文人工核对、用户流程 §10 全项审计与最终验收报告。沿用 LangGraph、当前代理独立实施与复核；不擅自恢复暂停的总目标或调用 Claude。
...
| 2 | 导入旧作 | ✅ 正文入库第 29 节；逐章反推第 31 节；多文件第 44 节；资料文件与人识别第 45 节；人物档案单独上传第 46 节；声明的关系进关系图第 47 节 |
...
**分支与 PR**
- `master` = `4c30080`（PR #20 / #21 / #22 三条按序合入 —— 资料文件收下与人识别、人物档案单独上传、声明的关系进关系图）。
- **PR #23 open**：分支 `l2-budget`，L2 硬预算与逐级裁剪（§48）。
- 本轮 `gh` 又断过一次（`api.github.com` 000/5.00s），过几小时自行恢复；断了就先把分支推上去、PR 稍后补开。
- 远端已 prune 掉合完的分支；留着的 `stage1-*` / `stage2-*` 是历史线，不删。
- 工程状态：**1280 测试过**（1 skipped），typecheck（根+web）、web:build、`git diff --check` 通过。
...
- **导入旧稿：资料文件是人物信息的主来源，正文只抽样**（§45、用户口径）。作者手上的角色档案比从几十万字正文里推断可靠也短得多，「正文不需要读全文」。资料文件随正文导入时收进 `materials/`，也可在资料页单独上传（§46）。
- **「他第一次出现在第几章」由代码扫正文得出**，不问模型（§45）—— 确定性的字符串查找，模型给不出更准的答案却可能给一个错的。
- **声明的关系落事件流、没有正文出处画虚线**（§47）：用 `anchor` 缺席表达「尚未写进正文」，不给空锚点充数（那会被解析成「原文已变动」，一个完全不同的结论）。**有正文出处的边永远压过只有声明的**，与先后无关。`authored` 不能用（它是「作者手填、模型不得覆盖」），走 proposed → committed。
- **L2 有硬预算**（§48）：`L2_TOKEN_BUDGET = 3000` + 逐级裁剪，只动远处梗概与次要角色窗口，人物/伏笔/情节线一律不裁。
- **卷的边界只有一份真相：`Beat.volume`**（§36）。`volumes.json` 只存卷名与卷纲，不存章号；范围由 `volumeRanges()` 推出。卷纲**顶替**远距离逐章梗概（不是叠加），且只顶替写完的卷、写了纲的卷、整卷落在远距离区的卷。

**待办（按序）**

1. **队列头 = 让弧线 / 伏笔时间线 / 情节线三张视图在导入后也有内容**（§47 末）。关系图这一批已经通了：关系从资料抽出、随方案确认后落事件流、没有正文出处的画虚线。那三张读的是 `character_presence` / `foreshadow_*` / `plot_event`，**同一个病、同一个路子**，可以照做。用户的《失踪档案》是最现成的验收集。
2. 记账留下的直接后续：余额拦截（用户已拍板「只看余额正负、允许小幅超支」，测试写在 stash 里）、充值入口、`pricing.yaml` 价格核对（**表里是占位价，上线前必须按官网核对**）。
...
5. ~~**L2 仍没有硬预算与裁剪**~~ ✅ 第 48 节（2026-09-21）：`L2_TOKEN_BUDGET = 3000` + 逐级裁剪，2000 章 38,223 → 2,709 tok。
...
9. **既有注释与文档可能是错的，别当事实用**：§13.4 那句「200 章压到 ~1000 tok」
   ...
   动手前实测一次比读注释可靠。
10. **读到「X 做不到 Y」时，下一句要问「那什么能做到 Y」**（§48）：§13.4 写着
    「分桶本身不压缩内容」，我照着它写了梯子却没照着它改算法，结果 2000 章仍然
    三万多 token。**那句话是警告，不是事实描述。**
11. **变异测试要确认它真的跑了**（§48）：`-t` 用的是旧测试名，匹配不到 → 跑 0 条
    → 退出码 0 → 假绿。改过测试名之后回头跑变异脚本，先看一眼它跑了几条。
12. **真机是唯一的验收标准**（§44–§47）：导入旧稿这条线上，**三处缺口全是真机撞出来的**
    （不收多文件 → 目录文件被当成书 → 不收文件夹），每一处单测都撞不出来。
    有真实素材（用户的《失踪档案》：105 个文件、100 章、29.7 万字）就用它。
...
## 45. 2026-09-21 导入收下资料文件 + 从正文与资料识别人物

分支 `character-intake`（base master `3fca9d7`，不栈）。**1260 测试过**（基线 1250 + 10），四件套干净，端到端 0 次真实调用。

### 起因

作者导入旧稿时，目录里除了 100 章正文还有角色档案、大纲、简介、审查规则/报告。此前那些文件被跳过就完了 —— 而**角色档案恰恰是这本书人物信息最可靠的来源**（比从 30 万字正文里推断可靠，也短得多）。用户明确说了：人物信息他会提供介绍文件，**正文不需要读全文**。

### 三件事

- **没被当成正文的文件收进作品资料**（新 `src/import/materials.ts`，`materials/` 目录）。作者的创作资产，与正文同等；识别人物要用，存下来也意味着以后再起草资料时它仍在场。落盘名与展示名分开：展示用原名（选整个文件夹时浏览器给的是带目录前缀的路径），落盘只取净化后的文件名 —— 这个进程能读用户主目录下的一切，路径穿越必须挡。**过大的资料在预览里说明并真的不收**。
- **识别人物：资料全文进，正文只抽样**（`src/preparation/sources.ts`）。一百章 30 万字塞不进上下文；**抽哪几段由代码定**（头部三章 + 其余均匀几章），**谁是主角由模型判断** —— 抽取与判断分开。提示词写明只抽了几章，免得模型声称读过全书。
- **首次出场章由代码扫正文得出**（`preparation/service.ts` 的 `firstAppearance`），模型不必也不该管 —— 确定性的字符串查找，模型给不出更准的答案却可能给错的。名字与别名都算；只对**新**人物算，已有人物原值不动（那是一份用过的事实，改了会让依赖它的东西莫名过期）。

识别人物不新增链路：复用既有的「起草」入口（`focus: "characters"`），产物仍是 **proposed 方案**，作者在资料页审。分类用既有的 `CharacterTier`（主角/主要/次要/龙套），不新增 schema 字段。

### 首页

空作品的首页此前是**死路**（0 章时只有「当前没有需要处理的结构债」和空的下一章）。现在：0 章 → 「导入已有正文」；有正文没人物 → 「从正文识别人物」。`overview` 加了 `characterCount` 供它判断。两者都指向既有弹窗与既有起草端点。

### 验证

- 10 个新测试，六条规则逐条变异测试。**「过大跳过」起初没被测到** —— 我只断言了提示文案，没断言它真的没落盘；只提示不拦等于提示是假的。
- **真机端到端**（作者的 105 个文件，脚本化替身 0 次调用）：导入 100 章 / 297165 字 / 收下 5 份资料 → 识别人物产出待确认方案 → **沈叙的首次出场章扫出为第 1 章**（正确）→ 正式资料未被改动 → 角色档案全文与抽样正文都送到了模型 → 提示词说明了只抽样。
- 浏览器：空书首页与有稿无人物首页两种状态、从首页打开导入弹窗（两个选择器都在）、390px 无溢出、无 console 错误。

## 46. 2026-09-21 人物档案单独上传（资料页）

分支 `material-upload`（base `character-intake`）。**1266 测试过**，四件套干净，浏览器端到端 0 次真实调用。

### 起因

作者手上有现成的角色档案，但**没有入口单独交给系统**：导入旧稿那条路要求至少带一个章节文件（整批都是资料会被「没有识别到章节标记」挡下），而资料页只有「让 AI 起草人物」（从想法推）与「手工补一个」。用户指定：**在人物档案那一栏加一个按钮**。

### 决策

- **上传即收下 + 立刻识别人物**（用户拍板）。作者点的是「导入人物档案」，不是「上传一个文件」；档案收下了却还要去别处再点一次，等于把话说了半句。产物仍是**待确认方案**，核对后才生效。
- **`MaterialStore.save` 从「返回 null」改成「带理由地拒收」**：作者可能选了一整个文件夹（里面有图片、zip），一句「没成功」让他不知道是哪一份、为什么。逐份给出「内容是空的 / 超过十万字上限 / 文件名不能用」。
- **一份失败不整批不收**，该收的收下、不该收的点名说清。
- **带路径的名字剥成文件名存下**（`../../角色档案.txt` → `角色档案.txt`）：既不越界，也不因为带路径丢掉作者的文件。（写测试时我一度断言它该被拒 —— 那是错的：`basename` 之后既安全又不该丢，同文件里先写的那条导入测试早就把它钉成「存下」，两条期望打架时是新的那条错。）
- **上传成功但整理人物失败时，先说清文件已经收下了**。原先只在整理成功后一起提示，模型那步一挂作者会以为白传了 —— 反馈必须说出**已经发生的事**，这条在浏览器实测里发现的。

### 接口

`GET /api/materials` 列出、`POST /api/materials` 上传。与随旧稿导入收下的**共用同一份存储与同一套拒收理由**，不另起一条链路。

### 验证

6 个新测试，六条规则逐条变异测试。浏览器端到端：按钮就位（人物档案栏里排第一）、真实上传落盘（8628 字节与原文件一致）、整理失败时文案如实、390px 无溢出、无 console 错误。`readTextFile`（UTF-8 严格解码回落 GBK）从导入组件提出来共用。

## 47. 2026-09-21 资料里声明的关系进关系图（画虚线）

分支 `relations-from-material`（base `material-upload`）。**1273 测试过**，四件套干净。真机跑过一次（真实模型）。

### 起因：真机验证撞出来的空白

用户要求「导入一下，看看页面内容与关系图对不对」。在临时作品里真跑（作者的 105 个文件、真实模型）：

- ✅ 导入 100 章 / 297165 字 / 收下 5 份资料；识别人物 **18 个**，tier 分档与首次出场章（代码扫出）都对；人物档案页 18 张卡
- ❌ **关系图空的**；人物弧线列出了人，但出场章数全 0、转折点全无

根因：**关系图的唯一数据源是事件流里的 `relation_changed`**，而那个只有写章 / 采用结构声明 / 逐章反推时才产生。作者的《角色档案》里明明有一份 20 条的关系表，**没有任何入口能进去**。

### 决策（用户从三个方案里选了 A）

| | 方案 | 取舍 |
| --- | --- | --- |
| **A（选定）** | 从资料抽关系，落成事件，图上与正文出处区分画法 | 立刻有内容；但没有正文出处的边不能跳原文 |
| B | 不抽，靠既有的「从正文反推结构」逐章补 | 带真实锚点、能跳原文；100 章要逐章点，几百积分 |
| C | 两者都做 | 最完整，改动最大 |

### 三处判断

- **用 `anchor` 缺席表达「尚未写进正文」，不给空锚点充数。** 空 quote 会被 `resolveAnchor` 解析成 `quote_not_found` —— 那是「原文已变动」，一个完全不同的结论。现在 `point` 为 null，图上画虚线、不是可跳转的点。
- **`authored` 不能用**（它按 `primitives.ts` 的定义是「作者手填、最高可信度、模型不得覆盖」，比这重得多）。模型读作者的文件抽出来的、作者确认后才成立，所以走 **proposed → committed**，`origin: material_import`。为此给 `EventStream` 加了 `decideIds` —— `decideChapter` 按章筛，而这类关系不属于任何一章，**按来源批量提升会把别处待确认的同类一起卷进来**。
- **有正文出处的边永远压过只有声明的**，与谁先谁后无关：资料那句是作者的设定，正文那句是已经写出来的事实。**这条第一版写反了**（写成「后者覆盖前者」），测试直接把它打回来。

顺带收紧契约：`C5Declaration.relationsChanged` 要求**必带锚点**（C5 是从刚写完的正文里读出来的），事件载荷的 `anchor` 才可选。`declarationOf` 相应地只收有锚点的关系。

### 真机结果

20 个人物 + **18 条关系**（双向各一条），确认后图上 18 条边全为虚线、12 个节点，消耗 8.35 积分。界面上虚线边带「仅设定」标签、「变于」列显示「—」，页头与图例都说清实线虚线之别；方案预览列出「这份方案会声明 N 条人物关系」（与「将删除」同一条理由：预览画不出来就得说出来）。

### 仍未解决

**人物弧线仍是空的**（出场章数 0、无转折点）—— 它读 `character_presence` / `character_state_changed`，与关系同病，但这一批没做。伏笔时间线与情节线同理。这是「导完一本书，四张视图三张是空的」这个更大的问题的一部分，下一批可以照同样的路子做。

## 48. 2026-09-21 L2 硬预算与逐级裁剪（差距第 6 项留下的尾巴）

分支 `l2-budget`（base master `4c30080`）。**1280 测试过**，四件套干净。

### 先量后做

§13.4 的注释里有个乐观估计（「每章约 7 tok」）。实测（同一份造数：一句话梗概 30–50 字、8 主要 + 30 次要角色、12 伏笔、3 情节线）：
...
实测约 19 tok/章，不是 7。卷纲那一批有效（压平了），**没有卷纲时仍线性涨**。设计文档 §13.4 表格给的目标区间是 2000–3000。

### 决策

- **`L2_TOKEN_BUDGET = 3000`**（区间上沿，用户拍板）。比 L3 的 5000 小 —— L2 是最大的可缓存前缀。
- **逐级裁剪**，与 `selectL3` 同构：一串档位从最无害试起。裁完：2000 章 38,223 → **2,709**。
- 档位只动**远距离梗概**与**次要角色窗口**；人物、伏笔、情节线一律不裁（它们是「要做的事」）。裁到底仍超预算时给 `overflowNote`，不再无限裁。

### ⚠ 一处差点做错：分桶不压缩内容

第一版梯子只放大远距离桶（15 → 30 → 60 → 120），2000 章**仍然三万多**。因为 §13.4 那句「分桶本身不压缩内容」说的就是这个：桶变大只是把 15 句并成 120 句，**总字数一条没少**，减的只是行数。

**真正的杠杆是每桶限长**（`condense` 的 `maxChars`，超限截断并留省略号让模型知道这段是残的）。两者一起用才是「越远越模糊」的完整含义：间隔更宽，每段也只剩一句半句。

**教训**：注释里那句话是对的，但它是**警告**不是事实描述 —— 我照着它写了梯子却没照着它改算法。读到「X 做不到 Y」时，下一句该问「那什么能做到 Y」。

### 验证

7 个新测试，五条规则逐条变异测试。两条起初没测到：①「收紧窗口」—— 默认预算下 2000 章停在 `far_60`，**根本走不到那一档**，改用「人物极多的书」（300 个次要角色都在近 20 章出现过）才钉住；②变异脚本的 `-t` 用了旧测试名，匹配不到就没跑 —— **假绿**，改名字后立刻变红。

三个新常量按既有规矩具名登记进 `test/rules.test.ts` 的 REGISTERED（L2 是布局不是故事阈值，与 `L1/L3_TOKEN_BUDGET` 同类）。

## 49. 2026-09-21 本轮概览：导入旧稿这条线怎么从撞墙走到通了四分之三

**这一节是索引与叙事，细节看它指向的那几节。** 本轮从「作者的旧稿导不进来」开始，做掉了五批，全部已合入 master（除 §48 外）。

### 起因：真机撞墙

用户拿自己的旧稿《失踪档案》（一个目录、105 个 `.txt`：100 章正文 + 角色档案/大纲/简介/审查规则/审查报告）来导入，导不进去。**这个真实素材贯穿了整轮**，三处缺口全是它撞出来的，每一处单测都撞不出来：

| 撞墙 | 原因 | 出处 |
| --- | --- | --- |
| 一本一百章要先手动拼成一个文件 | 入口只收单个文件或一段粘贴 | §44（多文件 + 整文件夹） |
| 拼好之后仍报「第 1 章出现了两次」 | `目录及简介.txt` 每行都像章节标记 → 被当成一本「每章都没正文的书」 | §44 |
| 系统对话框里选文件夹，「打开」是灰的 | 文件选择器不收文件夹 | §44 |

### 五批

1. **§44 多文件导入**：一次收一批，按文件名里的章号排序（不是字典序），无标记文件与目录文件跳过并点名。
2. **§45 资料文件 + 人识别**：没被当成正文的文件收进 `materials/`；识别人物时**资料全文进、正文只抽样**（100 章 30 万字塞不进上下文），**首次出场章由代码扫正文得出**；首页补了空作品的起步入口。
3. **§46 人物档案单独上传**：资料页人物栏加「导入人物档案」，上传即收下并立刻整理人物。
4. **§47 声明的关系进关系图**：关系原先唯一的来源是事件流的 `relation_changed`，而它只在写章/反推时产生 —— 所以人物满、关系图空。现在识别人物时一并抽关系，随同一份方案确认，**没有正文出处的画虚线**。
5. **§48 L2 硬预算与裁剪**：卷纲之外的另一半 —— 作者不写卷纲时 L2 仍线性涨（2000 章 38,223 tok），加 `L2_TOKEN_BUDGET = 3000` + 逐级裁剪，压到 2,709。

### 真机验收到哪一步

在临时作品里用真实模型跑过两轮（**共约 15 积分**）：

- 导入 100 章 / 297,165 字 / 收下 5 份资料 ✓
- 识别人物 **20 个**，档位分对、首次出场章扫对 ✓
- 关系 **18 条**，确认后关系图 18 条虚线边 / 12 个节点 ✓
- **人物弧线、伏笔时间线、情节线仍是空的** ✗ ← 队列头

### 用户本轮拍板的口径（勿再讨论）

- 人物信息以**作者提供的资料文件**为主，正文不需要读全文。
- 「分类」用既有的出场权重四档（主角/主要/次要/龙套），不新增 schema 字段。
- 识别人物**不自动跑**，给一键入口由作者点。
- 关系走「资料抽取 + 虚线区分画法」，**不靠逐章反推**（那条要几百积分）。回收站不给「彻底删除」（§38）；L2 预算取 3000（§48）。
- 登录**先不做**。

### 一条贯穿全轮的方法论

**注释里那句「X 做不到 Y」是警告，不是事实描述**（§48）。读到它时下一句该问「那什么能做到 Y」——我照着它写了梯子却没照着它改算法，白跑一轮。同类：**变异测试要确认它真的跑了**（`-t` 名字过时 → 跑 0 条 → 退出码 0 → 假绿）。
