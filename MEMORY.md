...
| 5 | 批量/连续创作 与 跨多章返修 | 🔵 **连续创作已完成（第 33 节，PR #8）；跨多章返修仍未做 —— 队列头** |
...
- `continuous-run`（连续创作，**PR #8 open**，base `works-first-screen`，栈在 PR #7 之上）。三条 PR 按 #6 → #7 → #8 顺序合。
...
**待办（按序）**
1. **review 并合入 PR #6、PR #7、PR #8**（按栈顺序）；合后删三条远端分支。
2. **差距第 5 项下半场「跨多章返修」** —— 队列头。改早章后要定位受影响段落并给修订建议，现在只把后续稿标过期。
3. ~~谋篇模式与逐章反推的真机 live~~ 已跑（gemini-3-flash，见 §31 / §32 末）。**Mac 有 `.env.local`**，别再写成只有 Windows 有。
